...
public class CameraFollow : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (angleStableTime >= requiredStableTime && playerController.movementInput != Vector3.zero)
        {
            ...
            Quaternion inputOffsetRotation = Quaternion.Euler(inputOffsetIntensity * inputDirection.y,
                inputOffsetIntensity * inputDirection.x, 0);
    ...
    public void CallChangeCameraModeToStatic(bool isUnfixed, Vector3 newCameraPosition, Quaternion newCameraRotation,
        float newCamSpeed, bool lookPlayer)
    {
    ...
    public void CallChangeCameraModeToFollowPlayer(bool stop, Vector3 newOffset, Quaternion newCameraRotation,
        float newCamSpeed)
    {
        if (coroutineIsRunning) StopCoroutine(runningCoroutine);
    ...
    private IEnumerator ChangeCameraModeToStatic(bool isUnfixed, Vector3 newCameraPosition,
        Quaternion newCameraRotation, float newCamSpeed, bool lookPlayer)
    {
    ...
    private IEnumerator ChangeCameraModeToFollowPlayer(bool stop, Vector3 newOffset, Quaternion newCameraRotation,
        float newCamSpeed)
    {
        coroutineIsRunning = true;
        yield return new WaitForSeconds(camDelay);
